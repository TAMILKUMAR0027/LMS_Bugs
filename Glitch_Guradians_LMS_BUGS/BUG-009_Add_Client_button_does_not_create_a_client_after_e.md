---
name: "🐞 Bug Report"
about: Report a defect or unexpected behavior in the application
title: "[BUG] Add Client button does not create a client after entering all mandatory fields"
labels: ["bug"]
assignees: [Vetrivel]
---
# 🐞 Bug Report (BUG-009)

## Summary
Add Client button does not create a client after entering all mandatory fields

---

## Environment
| Field | Value |
|-------|-------|
| Environment | QA |
| Browser | Chrome |
| Browser Version | Latest |
| OS | Windows 11 |
| Build / Version | v1.1 |
| Module | Dynamic field (Add client) |

---

## Severity
- [ ] Critical (System/Application crash)
- [x] High (Major functionality broken)
- [ ] Medium (Feature partially affected)
- [ ] Low (Minor UI/Cosmetic issue)

---

## Priority
- [ ] P0 - Immediate
- [x] P1 - High
- [ ] P2 - Medium
- [ ] P3 - Low

---

## Preconditions
User should be logged into the LMS application successfully.

---

## Steps to Reproduce
1.Launch the Learning Hub application.
2.Navigate to Dynamic Field Management.
3.Click on the Client Management tab.
4.Click the New Client button.
5.Enter valid data into all mandatory fields:
6.Click the Add Client button.

---

## Expected Result
Client Added

---

## Actual Result
No action

---

## Frequency
- [ ] Always
- [ ] Intermittent
- [ ] Rare
- [ ] Unable to Reproduce

---

## Impact


---

## Error Messages / Logs
```text
```

---

## Screenshots / Screen Recording
None provided

---

## Attachments
- [ ] Playwright HTML Report
- [ ] Cucumber HTML Report
- [ ] Allure Report
- [ ] Execution Logs
- [x] Screenshot(s)
- [ ] Screen Recording

---

## Reporter Details
| Field | Value |
|-------|-------|
| Reported By | Vetrivel B |
| Assigned To | Dev Team |
| Date Reported | 2026-07-09 00:00:00 |
| Sprint | 3 |
| Related Story / Task | |
| Related Pull Request | |
| Status | New |

---

## Checklist
- [ ] Bug reproduced successfully
- [ ] Expected and actual results verified
- [ ] Supporting evidence attached
- [ ] Environment details provided
- [ ] Steps to reproduce are complete
- [ ] Labels assigned appropriately
